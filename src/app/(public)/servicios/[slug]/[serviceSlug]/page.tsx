...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  const optionGroups = [
    { mode: "rental", title: "Alquiler" },
    { mode: "purchase", title: "Compra" },
  ].map((group) => ({ ...group, options: options.filter((option) => option.mode === group.mode) }));
  return <main>
    ...
    {options.length > 0 && <section className="border-y border-blue-100 bg-blue-50/60 py-16 sm:py-24" aria-labelledby="service-options"><div className="public-container"><h2 className="text-3xl font-black text-slate-950" id="service-options">Opciones comerciales</h2><div className="mt-9 space-y-12">{optionGroups.map((group) => group.options.length > 0 && <div key={group.mode}><h3 className="text-xl font-black text-orange-700">{group.title}</h3><div className="mt-5 grid gap-6 md:grid-cols-2 xl:grid-cols-3">{group.options.map((option) => <article className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm" key={option.id}><h4 className="text-2xl font-black">{option.title}</h4>{option.equipment_count !== null && <p className="mt-3 text-sm font-bold text-slate-600">{option.equipment_count} {option.equipment_count === 1 ? "equipo" : "equipos"}</p>}<p className="mt-5 text-3xl font-black text-blue-900">{ars.format(Number(option.price))}</p>{option.price_label && <p className="mt-1 text-sm text-slate-500">{option.price_label}</p>}{option.description && <p className="mt-5 leading-7 text-slate-600">{option.description}</p>}</article>)}</div></div>)}</div></div></section>}
