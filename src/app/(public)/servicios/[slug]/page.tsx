...
import { getPublicServiceAreaBySlug, getPublicServicesByArea, serviceHref } from "@/lib/supabase/services";
...
export default async function ServiceAreaPage({ params }: Props) {
  ...
  return <main>
    ...
    <section className="public-container py-16 sm:py-24" aria-labelledby="area-services"><h2 className="text-3xl font-black text-slate-950 sm:text-4xl" id="area-services">Servicios del área</h2>{services.unavailable ? <p className="public-empty-state">Los servicios no están disponibles temporalmente.</p> : services.data.length === 0 ? <p className="public-empty-state">Estamos actualizando los servicios de esta área.</p> : <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">{services.data.map((service) => <article className="flex flex-col rounded-3xl border border-slate-200 bg-white p-7 shadow-sm" key={service.id}><ServiceAreaIcon icon={service.icon}/><h3 className="mt-6 text-2xl font-black text-slate-950">{service.name}</h3>{service.short_description && <p className="mt-3 leading-7 text-slate-600">{service.short_description}</p>}{service.description && <p className="mt-3 text-sm leading-6 text-slate-500">{service.description}</p>}<Link className="mt-6 inline-flex w-fit font-bold text-blue-800 underline decoration-orange-500 decoration-2 underline-offset-4" href={serviceHref(area.slug, service.slug)}>Conocer más</Link></article>)}</div>}</section>
