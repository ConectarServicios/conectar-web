...
import { toggleNewsFeatured, updateNewsState } from "@/app/admin/news/actions";
...
export default async function NewsAdminPage({ searchParams }: Readonly<{ searchParams: Promise<{ success?: string; error?: string }> }>) {
  ...
  return <>
    <div className="flex flex-col gap-4 sm:flex-row sm:justify-between"><AdminPageHeader description="Creá, programá y administrá las novedades institucionales." title="Noticias / Comunicados"/><Link className="shrink-0 rounded-xl bg-orange-600 px-5 py-3 text-center font-bold text-white" href="/admin/news/new">Nueva noticia</Link></div>
    ...
    <div className="grid gap-4 xl:block xl:overflow-hidden xl:rounded-2xl xl:border xl:bg-white"><div className="hidden grid-cols-[1.4fr_.7fr_.6fr_.8fr_.5fr_.8fr_1.7fr] gap-3 bg-slate-50 px-5 py-3 text-xs font-bold uppercase text-slate-600 xl:grid"><span>Título</span><span>Categoría</span><span>Estado</span><span>Publicación</span><span>Destacada</span><span>Autor</span><span>Acciones</span></div>{news.map((item) => <article className="rounded-2xl border bg-white p-5 shadow-sm xl:grid xl:grid-cols-[1.4fr_.7fr_.6fr_.8fr_.5fr_.8fr_1.7fr] xl:items-center xl:gap-3 xl:rounded-none xl:border-0 xl:border-b xl:shadow-none" key={item.id}>
      <h2 className="font-bold">{item.title}</h2><p className="mt-2 text-sm xl:mt-0">{item.category ?? "—"}</p><span className="mt-2 w-fit rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold xl:mt-0">{labels[item.status]}</span><p className="mt-2 text-sm xl:mt-0">{date(item.published_at)}</p><p className="mt-2 text-sm xl:mt-0">{item.featured ? "Sí" : "No"}</p><p className="mt-2 text-sm xl:mt-0">{item.author?.full_name ?? "—"}</p>
      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm xl:mt-0"><Link className="font-bold text-orange-700" href={`/admin/news/${item.id}/edit`}>Editar</Link><form action={updateNewsState}><input name="id" type="hidden" value={item.id}/><input name="status" type="hidden" value={item.status === "published" ? "draft" : "published"}/><button className="font-bold text-slate-700">{item.status === "published" ? "A borrador" : "Publicar"}</button></form>{item.status !== "archived" && <form action={updateNewsState}><input name="id" type="hidden" value={item.id}/><input name="status" type="hidden" value="archived"/><button className="font-bold text-slate-700">Archivar</button></form>}<form action={toggleNewsFeatured}><input name="id" type="hidden" value={item.id}/><input name="featured" type="hidden" value={String(!item.featured)}/><button className="font-bold text-slate-700">{item.featured ? "Desmarcar" : "Destacar"}</button></form><NewsDeleteButton id={item.id} title={item.title}/></div>
    </article>)}</div>}
  </>;
}
