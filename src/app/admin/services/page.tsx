  ...
  return <>
    ...
    {error ? <div className="rounded-2xl border border-red-200 bg-white p-8 text-center"><h2 className="font-bold text-slate-950">No pudimos cargar los servicios</h2><p className="mt-2 text-slate-600">Intentá nuevamente en unos minutos.</p></div>
      ...
      : <div className="grid gap-4 lg:block lg:rounded-2xl lg:border lg:border-slate-200 lg:bg-white lg:shadow-sm">
        <div className="hidden grid-cols-[1.25fr_.75fr_1.6fr_.55fr_.55fr_.4fr_1.25fr] gap-2 border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-bold tracking-wide text-slate-600 uppercase lg:grid"><span>Servicio</span><span>Área</span><span>Descripción corta</span><span>Estado</span><span>Destacado</span><span>Orden</span><span>Acciones</span></div>
        {services.map((service) => <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:grid lg:grid-cols-[1.25fr_.75fr_1.6fr_.55fr_.55fr_.4fr_1.25fr] lg:items-center lg:gap-2 lg:rounded-none lg:border-0 lg:border-b lg:p-4 lg:shadow-none last:lg:border-b-0" key={service.id}>
