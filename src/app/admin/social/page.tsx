...
import { toggleSocialLink } from "@/app/admin/social/actions";
import { AdminPageHeader } from "@/components/admin/admin-page-header";
import { SocialLinkDeleteButton } from "@/components/admin/social/social-link-delete-button";
    ...
    {error ? <div className="rounded-2xl border border-red-200 bg-white p-8 text-center"><h2 className="font-bold text-slate-950">No pudimos cargar las redes sociales</h2><p className="mt-2 text-slate-600">Intentá nuevamente en unos minutos.</p></div>
      ...
      : <div className="grid gap-4 lg:block lg:overflow-hidden lg:rounded-2xl lg:border lg:border-slate-200 lg:bg-white lg:shadow-sm">
        ...
        {socialLinks.map((socialLink) => <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:grid lg:grid-cols-[1fr_2fr_.7fr_.5fr_1.3fr] lg:items-center lg:gap-3 lg:rounded-none lg:border-0 lg:border-b lg:p-5 lg:shadow-none last:lg:border-b-0" key={socialLink.id}>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm lg:mt-0"><Link className="font-bold text-orange-700 hover:text-orange-900" href={`/admin/social/${socialLink.id}/edit`}>Editar</Link><form action={toggleSocialLink}><input name="id" type="hidden" value={socialLink.id} /><input name="active" type="hidden" value={String(!socialLink.active)} /><button className="font-bold text-slate-700 hover:text-slate-950" type="submit">{socialLink.active ? "Desactivar" : "Activar"}</button></form><SocialLinkDeleteButton id={socialLink.id} platform={socialLink.platform} /></div>
        </article>)}
      </div>}
  </>;
}
