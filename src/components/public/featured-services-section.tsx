  ...
  return <section className="border-t border-blue-100 bg-blue-50/60 py-16 sm:py-24" aria-labelledby="featured-services-title"><div className="public-container">
    ...
    <div className="mt-10 grid gap-7 md:grid-cols-2 xl:grid-cols-3">{services.map((service) => {
      ...
      return <article className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg" key={service.id}>
        <div className="relative grid aspect-[16/10] place-items-center overflow-hidden bg-[#0b2440]">{managedImageUrl ? <Image alt={service.service_media?.[0]?.alt_text ?? service.name} className="object-cover transition duration-300 group-hover:scale-105" fill sizes="(min-width: 1280px) 33vw, (min-width: 768px) 50vw, 100vw" src={managedImageUrl} /> : <LegacyServiceImage alt={service.name} className="size-full object-cover transition duration-300 group-hover:scale-105" fallback={<ServiceAreaIcon icon={service.icon} />} src={service.image_url} />}</div>
