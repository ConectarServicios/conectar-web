...
export function PromotionCard({ item, imageUrl, compact = false }: Readonly<{
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-lg shadow-slate-950/8">
      <div className="relative h-48 bg-gradient-to-br from-brand-navy to-[#164b75]">
        {imageUrl ? <Image alt={`Imagen de la promoción ${item.title}`} className="object-cover transition duration-300 group-hover:scale-[1.02]" fill sizes="(max-width: 768px) 100vw, 33vw" src={imageUrl} unoptimized />
      ...
      <div className="p-6">
        <h3 className="text-2xl font-black tracking-tight text-brand-navy-deep"><Link href={`/promociones/${item.slug}`}>{item.title}</Link></h3>
