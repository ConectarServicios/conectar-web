...
export function parseHeroForm(formData: FormData): { data?: Omit<HeroSlideFormValues, "image_path">; errors: Record<string, string> } {
  ...
  const buttonUrl = String(formData.get("button_url") ?? "") || null;
  ...
  if (buttonUrl && !isSafePublicNavigationUrl(buttonUrl)) errors.button_url = "Usá una ruta interna que comience con / o una URL http/https válida.";
